					...
					{	
						...
						if(!isset($row['c'])){
								...
								if($recLevel==0){
									...
									$relations[$extractedOid][$row['b']][0]=new RDFObject(NULL, $this, $extractedOid, $recLevel-1);
									$relations[$extractedOid][$row['b']][1]=-1;
							...
							if(!(strpos($row['c'], "oid")===false) && $row['b'] != "ca:hasOid"){
									if($recLevel==0){
										...
										$relations[$this->extractOid($row['c'])][$row['b']][0]=new RDFObject(NULL, $this, $this->extractOid($row['c']), $recLevel-1);
										$relations[$this->extractOid($row['c'])][$row['b']][1]=1;
...
function getClasses($searchString){
		...
		while($row = odbc_fetch_array($result)){
				...
				$pushArray = array('class' => $row['c'], 'label'=> $label, 'levenshtein' => levenshtein($searchString, $label) );
...
function getAllPropertyTypes($searchString, $type = NULL){
		...
		$resultArray=array();
		while($row = odbc_fetch_array($result)){
				if($row['c'] == "ca:hasOid" || !(strpos($row['c'], "instance")===false)){continue;}
